...
（例：ラズパイ$\rightarrow$小型コンピュータ）
 ...
 \item 主語と述語，形容詞と目的語など，関連の強い語句同士はできるだけ近くに記述している． \\
 \item すべての文の助詞（てにをはが）は適切に使用している．
 \item 適切な接続詞を用いて，文と文のつながりが明確である．
 \item 「既知未知法」に沿って，既知の情報を先に，未知の情報を後に記述している．
\end{itemize}
...
\begin{itemize}
 ...
 \item 参考文献は「著者名」「論文・発表題目」「論文誌名または発表会議名」「ページ番号または発表番号」「発表年」の情報がもれなく記述されている．記法は各投稿先の指定に順ずること．
...
\appendix
\section*{（付録）参考文献の書き方}
参考文献の書き方はそれぞれの指定の様式に順ずる記法を採用します．ここでは日本ロボット学会誌の様式を記載します．
...
\begin{itemize}
\item 和文の著者名は原則，名字のみを記載する．
\item 英文の著者名は原則，first name はイニシャルのみで，family name を記載する．
\item 和文の著者が3名以上の場合には，高橋ら，のように「ら」をつけて省略できる．
\item 英部の著者名が3名以上の場合には，T.~Takahashi et al. のように「et al.」をつけて省略できる（ピリオド，スペースの記法に注意）
\end{itemize}
